"""JPMorgan Chase scraper — uses Oracle HCM Cloud's public CandidateExperience REST API."""
...
_BASE = "https://jpmc.fa.oraclecloud.com/hcmRestApi/resources/latest"
_SITE = "CX_1001"
_PAGE_SIZE = 25
...
class JPMorganScraper:
    ...
    def __init__(self, company_name: str = "JPMorgan Chase Hong Kong"):
        self.company_name = company_name
        self.board_token = "oracle-hcm-jpmc"  # for scrape_all.py logging
    ...
    def get_job_details(self, job_id: str) -> Dict:
        try:
            ...
            if not items:
                return {}
            job = items[0]
            description = strip_html(job.get("ExternalDescriptionStr", ""))
            quals = strip_html(job.get("ExternalQualificationsStr", ""))
            resp_text = strip_html(job.get("ExternalResponsibilitiesStr", ""))
            parts = [p for p in [description, quals, resp_text] if p]
            return {"description": "\n\n".join(parts) if parts else None}
        except Exception as e:
            print(f"Error fetching JPM job {job_id}: {e}")
            return {}

    def _fetch_page(self, offset: int, location: str) -> Optional[Dict]:
        # Build URL manually — requests.get(params=...) encodes ; and , breaking Oracle's finder syntax
        ...
        for attempt in range(3):
            try:
                ...
                if attempt < 2:
                    ...
                    print(f"Error fetching JPM page offset={offset}: {e}")
                    return None
    ...
    def _parse_listing(self, item: Dict) -> Dict:
        ...
        return {
            ...
            "url": f"https://jpmc.fa.oraclecloud.com/hcmUI/CandidateExperience/en/sites/{_SITE}/jobs/preview/{job_id}",
